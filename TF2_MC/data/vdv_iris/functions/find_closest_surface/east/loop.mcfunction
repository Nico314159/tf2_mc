#> vdv_iris:find_closest_surface/east/loop
#
# Determines if the ray hits the current surface
#
# @within vdv_iris:find_closest_surface/east/main
...
execute if score ${x} vdv_iris > $x0 vdv_iris run function vdv_iris:find_closest_surface/east/get_distance
...
execute if score $surfaces vdv_iris matches 1.. run function vdv_iris:find_closest_surface/east/loop