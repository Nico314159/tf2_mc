...
summon arrow ~ ~1 ~ {Tags:["projectile", "smg"], damage:0.3d, pickup:0b, NoGravity:1b}

# assign projectiles to a team
data modify entity @e[tag=projectile,limit=1] Owner set from entity @s UUID

# apply motion to projectile
execute store result entity @e[type=arrow,tag=projectile,limit=1] Motion[0] double 0.0033 run scoreboard players get #targetX pos
execute store result entity @e[type=arrow,tag=projectile,limit=1] Motion[1] double 0.0033 run scoreboard players get #targetY pos
execute store result entity @e[type=arrow,tag=projectile,limit=1] Motion[2] double 0.0033 run scoreboard players get #targetZ pos

# clean up, ready for the next player
scoreboard players set @s attack_cooldown 0
