...
execute as @e[type=marker,tag=explode] at @s run function tf:find_pair
...
execute as @s unless score @s rocketsInHand = @s primaryClip unless entity @s[tag=reloading_rocket] unless entity @s[nbt={Inventory: [{Slot: -106b, tag:{isRocketLauncher:1b}}]}] run function tf:fix_rocket_clip
