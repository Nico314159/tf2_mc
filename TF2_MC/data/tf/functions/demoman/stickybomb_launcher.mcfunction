...
execute if entity @s[team=RED] positioned ~ ~-0.3 ~ anchored eyes run summon snowball ^-0.33 ^ ^0.66 {Tags:["projectile", "deflectable_projectile", "red_stickybomb", "unarmed_stickybomb"], Item:{Count:1b,id:"paper",tag:{CustomModelData:1}}}
execute if entity @s[team=BLU] positioned ~ ~-0.3 ~ anchored eyes run summon snowball ^-0.33 ^ ^0.66 {Tags:["projectile", "deflectable_projectile", "blu_stickybomb", "unarmed_stickybomb"], Item:{Count:1b,id:"paper",tag:{CustomModelData:1}}}

# summon matching armor stand
execute if entity @s[team=RED] at @s positioned ~ ~-0.3 ~ run summon armor_stand ^-0.33 ^ ^1 {Tags:["projectile", "red_stickybomb", "unarmed_stickybomb", "deflectable_projectile"], Invisible:1b,Invulnerable:1b,NoBasePlate:1b,NoGravity:1b,Rotation:[0.0f,0.0f],Marker:1b,ArmorItems:[{},{},{},{id:"paper",Count:1b,tag:{CustomModelData:230000}}],HandItems:[{},{}],CustomNameVisible:0b,Pose:{Head:[1.0f,1.0f,1.0f]}}
execute if entity @s[team=BLU] at @s positioned ~ ~-0.3 ~ run summon armor_stand ^-0.33 ^ ^1 {Tags:["projectile", "blu_stickybomb", "unarmed_stickybomb", "deflectable_projectile"], Invisible:1b,Invulnerable:1b,NoBasePlate:1b,NoGravity:1b,Rotation:[0.0f,0.0f],Marker:1b,ArmorItems:[{},{},{},{id:"paper",Count:1b,tag:{CustomModelData:240000}}],HandItems:[{},{}],CustomNameVisible:0b,Pose:{Head:[1.0f,1.0f,1.0f]}}

# assign projectiles to a team
data modify entity @e[tag=projectile,limit=1,type=snowball] Owner set from entity @s UUID
# data modify entity @e[tag=projectile,limit=1,type=armor_stand] Owner set from entity @e[tag=projectile,limit=1,type=snowball] Owner

scoreboard players operation #tempThree pairedOwnerUUID = @s converted-uuid0
execute as @e[tag=projectile,limit=1,type=armor_stand, tag=unarmed_stickybomb] run scoreboard players operation @s pairedOwnerUUID = #tempThree pairedOwnerUUID
...
execute as @s at @s run function tf:demoman/rotate_stand


# apply motion to projectile
execute store result entity @e[type=snowball,tag=projectile,limit=1] Motion[0] double 0.0005 run scoreboard players get #targetX pos
execute store result entity @e[type=snowball,tag=projectile,limit=1] Motion[1] double 0.0005 run scoreboard players get #targetY pos
execute store result entity @e[type=snowball,tag=projectile,limit=1] Motion[2] double 0.0005 run scoreboard players get #targetZ pos

# clean up, ready for the next player
scoreboard players set @s attack_cooldown 12
scoreboard players remove @s secondaryClip 1
function tf:demoman/fill_clip
