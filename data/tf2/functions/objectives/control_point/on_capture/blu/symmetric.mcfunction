...
execute as @e[type=marker,tag=tf2.current,scores={tf2.team=1}] if score @s tf2.index = $temp tf2.index run tag @s remove tf2.locked
...
execute unless entity @s[scores={tf2.team=0}] as @e[type=marker,tag=tf2.current,scores={tf2.team=2}] if score @s tf2.index = $temp tf2.index run tag @s add tf2.locked
