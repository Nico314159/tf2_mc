#> tf2:objectives/control_point/set_owner/red/symmetric
# 
...
execute as @e[type=marker,tag=tf2.current,scores={tf2.team=2}] if score @s tf2.index = $temp tf2.index run scoreboard players set @s tf2.locked 0
...
execute unless entity @s[scores={tf2.team=0}] as @e[type=marker,tag=tf2.current,scores={tf2.team=1}] if score @s tf2.index = $temp tf2.index run scoreboard players set @s tf2.locked 1
