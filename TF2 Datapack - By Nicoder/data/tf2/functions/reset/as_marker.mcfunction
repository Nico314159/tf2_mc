execute if score $Settings.show_debug_messages tf2.var matches 1 run tellraw @a ["",{"text":"<Debug> ","bold":true},{"text":" Game controlled by ","bold":false},{"selector":"@s"},{"text":" has ended at "},{"score":{"name":"$__global__","objective":"__player_join__"}},{"text":" ticks gametime"}]
