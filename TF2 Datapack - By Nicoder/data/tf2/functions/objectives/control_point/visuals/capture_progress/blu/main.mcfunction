...
execute if score @s[scores={tf2.blu_progress=0}] tf2.team matches 0 run data modify storage tf2:lookup points[0].owner set value "[{\"text\":\"[\",\"color\":\"#F5EACB\"},{\"text\":\"3210=\",\"color\":\"#A1A1A1\"}]"
execute if score @s[scores={tf2.blu_progress=0}] tf2.team matches 0 run data modify storage tf2:lookup points[0].progress set value "[{\"text\":\"321))))))))))))))\",\"color\":\"gray\"}]"
