...
execute if score @s[tag=!tf2.said_death_msg] tf2.health matches 1.. run return run playsound tf2:ding.hit player @a[tag=retina.executing,predicate=!tf2:holding_melee] ~ ~ ~ 0.9 1 0.05
scoreboard players operation $local tf2.batch_number = @s tf2.batch_number
execute as @a if score @s tf2.batch_number = $local tf2.batch_number run tag @s add tf2.current
scoreboard players set __if_else__ tf2.var 0
execute if score $_isHeadshot_ tf2.var matches 1.. run function tf2:__private__/if_else/15
