execute if score $Settings.show_debug_messages tf2.var matches 1.. run tellraw @a ["",{"text":"<Debug> ","bold":true},{"text":"Player ","bold":false},{"selector":"@s"},{"text":" has logged in at "},{"score":{"name":"$__global__","objective":"0008it54g_p_join"}},{"text":" ticks gametime."}]
