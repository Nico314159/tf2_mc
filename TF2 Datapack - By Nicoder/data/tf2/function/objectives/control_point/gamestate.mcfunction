...
execute store result score $red_cp_count tf2.var if entity @e[type=item_display,tag=tf2.current,scores={tf2.team=1}]
execute store result score $blu_cp_count tf2.var if entity @e[type=item_display,tag=tf2.current,scores={tf2.team=2}]
...
tag @e[type=item_display,tag=tf2.control_point,tag=tf2.current] remove tf2.current