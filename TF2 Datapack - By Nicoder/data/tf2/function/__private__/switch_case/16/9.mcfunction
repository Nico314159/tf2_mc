...
tag @s add self
execute if score @s tf2.current_slot matches 1 unless score @s tf2.attack_delay matches 1.. as @e[type=#tf2:player_like,tag=!self] if score @s tf2.player.id = @p[tag=self] tf2.heal_target run function tf2:class/medic/apply_healing
tag @s remove self