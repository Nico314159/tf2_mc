...
execute if score $Settings.show_debug_messages tf2.var matches 1.. run tellraw @a ["",{"text":"<Debug> ","bold":true,"type":"text"},{"text":"Player ","bold":false,"type":"text"},{"selector":"@s","type":"selector"},{"text":" has logged in at "},{"score":{"name":"$__global__","objective":"xrroou1m8_p_join"},"type":"score"},{"text":" ticks gametime."}]
...
scoreboard players reset @s tf2.disguise
