...
execute as @a[tag=iris.targeted_entity,limit=1] if score @s tf2.team = $current tf2.team run return fail
...
data modify storage tf2:lookup enemy_pos set from entity @a[tag=iris.targeted_entity,limit=1] Pos
execute at @s summon marker run tag @s add get_pos
execute as @e[type=marker,tag=get_pos] run tp @s ~ ~ ~ facing entity @a[tag=iris.targeted_entity,limit=1] feet
...
execute as @a[tag=iris.targeted_entity,limit=1] run function tf2:math/facing_vector
