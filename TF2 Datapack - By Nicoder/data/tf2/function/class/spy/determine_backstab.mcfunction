...
execute if entity @a[tag=iris.target,gamemode=spectator] run return fail
scoreboard players operation $current tf2.team = @s tf2.team
execute as @a[tag=iris.target,limit=1] if score @s tf2.team = $current tf2.team run return fail
execute store result score $enemy_dist tf2.var run data get storage iris:output Distance 10
...
data modify storage tf2:lookup enemy_pos set from entity @a[tag=iris.target,limit=1] Pos
execute at @s summon marker run tag @s add get_pos
execute as @e[type=marker,tag=get_pos] run tp @s ~ ~ ~ facing entity @a[tag=iris.target,limit=1] feet
...
execute as @a[tag=iris.target,limit=1] run function tf2:math/facing_vector
