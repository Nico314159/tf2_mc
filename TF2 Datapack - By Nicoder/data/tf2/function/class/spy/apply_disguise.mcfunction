...
scoreboard players operation @s tf2.disguise.team = @s tf2.disguise
scoreboard players operation @s tf2.disguise.team /= 10 tf2.const
execute if score @s tf2.disguise.team matches 1 run data modify storage tf2:disguise team set value 'red'
execute if score @s tf2.disguise.team matches 2 run data modify storage tf2:disguise team set value 'blu'
execute unless score @s tf2.disguise.team matches 1..2 store result score $remove_disguise tf2.var run tellraw @s {"text":"ERROR: Invalid number. Server restart may be necessary.","color":"yellow","italic":true,"type":"text"}
execute if score $class tf2.var matches 1 if score @s tf2.disguise.team = @s tf2.team run scoreboard players set $remove_disguise tf2.var 1
