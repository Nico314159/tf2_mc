execute store result score $fake_CMD tf2.var run data get entity @s Inventory[{Slot:0b}].components.minecraft:custom_model_data 10
...
scoreboard players add $fake_CMD tf2.var 1
...
scoreboard players add $fake_CMD tf2.var 300000000
execute if score @s tf2.disguise.class matches 1 run scoreboard players set $fake_CMD tf2.var 400000000
item modify entity @s hotbar.3 {"function":"minecraft:set_custom_model_data","value":{"type":"minecraft:score","target":{"type":"minecraft:fixed","name":"$fake_CMD"},"score":"tf2.var"}}