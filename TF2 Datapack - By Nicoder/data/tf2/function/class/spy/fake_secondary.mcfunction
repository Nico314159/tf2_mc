execute store result score $merge_CMD tf2.var run data get entity @s Inventory[{Slot:1b}].components.minecraft:custom_model_data
...
scoreboard players add $world_CMD tf2.var 1
...
item modify entity @s hotbar.1 {"function":"minecraft:set_custom_model_data","value":{"type":"minecraft:score","target":{"type":"minecraft:fixed","name":"$merge_CMD"},"score":"tf2.var"}}