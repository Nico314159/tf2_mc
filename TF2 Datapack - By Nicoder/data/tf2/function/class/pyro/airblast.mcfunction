...
execute as @e[type=#tf2:player_like] if score @s tf2.batch_number = $current tf2.batch_number run tag @s add tf2.current
tag @s add origin
execute as @e[type=#tf2:player_like,tag=tf2.current] facing entity @s feet positioned ^ ^ ^1 rotated as @p[tag=origin] positioned ^ ^ ^-1 if entity @p[distance=..0.6,tag=origin] run tag @s add tf2.within_cone
