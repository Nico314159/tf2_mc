...
execute as @e[type=minecraft:item_display,tag=tf2.projectile,tag=tf2.stuck,tag=iris.targeted_entity] unless score @s tf2.team = @n[type=#tf2:player_like,tag=iris.executing] tf2.team run function tf2:projectile/fizzle