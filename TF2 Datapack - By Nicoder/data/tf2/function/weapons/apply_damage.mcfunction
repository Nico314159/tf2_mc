...
execute if score $backstab tf2.var matches 1.. run data modify storage tf2:lookup damages[-1].kill_verb set value 'backstabbed'
...
execute store result storage tf2:lookup damages[-1].cause int 1 run scoreboard players get @a[tag=retina.executing,limit=1] tf2.player.id
...
execute if score __if_else__ tf2.var matches 0 unless data storage tf2:lookup damages[-1].kill_verb run data modify storage tf2:lookup damages[-1].kill_verb set value 'killed'