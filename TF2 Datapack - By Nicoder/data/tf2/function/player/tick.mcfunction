...
tag @s add tf2.self
execute if predicate tf2:empty_hand run function tf2:__private__/if_else/58
tag @s remove tf2.self
function tf2:weapons/slot_check
scoreboard players set __if_else__ tf2.var 0
execute if items entity @s weapon.mainhand crossbow[charged_projectiles=[],custom_data~{should_detect_click:true}] run function tf2:__private__/if_else/59
...
item replace entity @s[scores={tf2.class=1..9},tag=!tf2.in_disguise_menu] hotbar.8 with carrot_on_a_stick[item_model="tf2:choose_class",custom_name='"Change Class"']
execute store result score $_count_ tf2.var run clear @s carrot_on_a_stick[item_model="tf2:choose_class",custom_name='"Change Class"'] 0
execute unless score $_count_ tf2.var matches 1 if entity @s[tag=!tf2.in_disguise_menu] run clear @s carrot_on_a_stick[item_model="tf2:choose_class",custom_name='"Change Class"']
execute unless score $_count_ tf2.var matches 1 if entity @s[tag=!tf2.in_disguise_menu] run item replace entity @s[scores={tf2.class=1..9}] hotbar.8 with carrot_on_a_stick[item_model="tf2:choose_class",custom_name='"Change Class"']
clear @s[tag=tf2.in_disguise_menu] carrot_on_a_stick[item_model="tf2:choose_class",custom_name='"Change Class"']
...
execute if score @s tf2.air matches -19 run function tf2:__private__/if_else/61
scoreboard players add @s[predicate=!tf2:submerged] tf2.time_surfaced 1
execute if score @s tf2.time_surfaced matches 40 run function tf2:__private__/if_else/62
...
execute if score @s tf2.team = @e[type=item_display,tag=tf2.resupply,distance=..1.5,limit=1] tf2.team if score $tick_cycle tf2.var matches 0 run function tf2:__private__/if_else/63
