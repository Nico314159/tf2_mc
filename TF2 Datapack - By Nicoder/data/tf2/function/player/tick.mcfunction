...
tag @s add tf2.self
execute if predicate tf2:empty_hand run function tf2:__private__/if_else/28
tag @s remove tf2.self
function tf2:weapons/slot_check
scoreboard players set __if_else__ tf2.var 0
execute if items entity @s weapon.mainhand crossbow[charged_projectiles=[],custom_data~{should_detect_click:true}] run function tf2:__private__/if_else/29
...
execute if entity @s[scores={tf2.class=0},predicate=tf2:active_offhand] run function tf2:__private__/if_else/30
...
execute if score @s tf2.air matches -19 run function tf2:__private__/if_else/32
scoreboard players add @s[predicate=!tf2:submerged] tf2.time_surfaced 1
execute if score @s tf2.time_surfaced matches 40 run function tf2:__private__/if_else/33
...
execute if score @s tf2.team = @e[type=item_display,tag=tf2.resupply,distance=..1.5,limit=1] tf2.team if score $tick_cycle tf2.var matches 0 run function tf2:__private__/if_else/34
